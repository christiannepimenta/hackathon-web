...
export default function Judge() {
  ...
  return (
    <main>
      ...
      <label>Email do juiz: <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="juiz@ex.com" /></label><br/>
      ...
      <h3>Pitch (0–100 cada critério)</h3>
      ...
      <div>Viabilidade: <input type="range" min="0" max="100" value={p4} onChange={e=>setP4(Number(e.target.value))}/> {p4}</div>
      <div>Extra (opcional): <input type="range" min="0" max="100" value={px} onChange={e=>setPx(Number(e.target.value))}/> {px}</div>
      <button onClick={()=>post({
